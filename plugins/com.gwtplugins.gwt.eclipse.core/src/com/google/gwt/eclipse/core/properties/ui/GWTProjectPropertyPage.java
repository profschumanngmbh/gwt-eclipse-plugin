...
/**
 * Property page for setting GWT project properties (SDK selection and the default set of entry
 * point modules).
 */
...
public class GWTProjectPropertyPage extends AbstractProjectPropertyPage {
  ...
  private void addEventHandlers() {
    ...
    /*
     * TODO: Add event handler on the ProjectSdkSelectionBlock to detect when the user has selected
     * a GWT SDK that would lead to an incompatible project configuration (i.e. using pre-GWT 1.6
     * with a project that has the WAR Nature).
     */
  }
  ...
  private void createSdkComponent(Composite parent) {
    ...
    // BEGIN CUSTOMIZATION //
    // Expose a textbox in the UI to set and show the fixed SDK version.
    // In practice, we actually set the property value directly via our Gradle plugin, so it's not really needed here.
  ...
  public void removeGWT() throws BackingStoreException, CoreException {
    ...
    ClasspathUtilities.replaceContainerWithClasspathEntries(getJavaProject(), GWTRuntimeContainer.CONTAINER_ID);
    GWTProjectProperties.setFileNamesCopiedToWebInfLib(getProject(), Collections.<String>emptyList());
  ...
  private void saveChangesToEntryPointModules() throws BackingStoreException {
    ...
    GWTProjectProperties.setEntryPointModules(getProject(), userEntryPointModules);
  }
  ...
  private void updateControls() {
    ...
    ExtensionQuery<GWTProjectPropertyPage.GwtSdkSelectionEnablementFinder> extQuery =
        new ExtensionQuery<GWTProjectPropertyPage.GwtSdkSelectionEnablementFinder>(GWTPlugin.PLUGIN_ID,
            "gwtSdkSelectionEnablementFinder", "class");
    ...
    for (ExtensionQuery.Data<GWTProjectPropertyPage.GwtSdkSelectionEnablementFinder> enablementFinder : enablementFinders) {
      shouldEnable = enablementFinder.getExtensionPointData().shouldEnableGwtSdkSelection(javaProject.getProject());
    }
    if (shouldEnable) {
  ...
  private IStatus validateSdk() {
    ...
    if (!GWTProjectsRuntime.isGWTRuntimeProject(javaProject)) {
      if (sdkSelectionBlock.getSdks().isEmpty()) {
        return StatusUtilities.newErrorStatus("Please configure an SDK", GWTPlugin.PLUGIN_ID);
      }
    }
    /*
     * TODO: Add validation to detect when the user has selected a GWT SDK that would lead to an
     * incompatible project configuration (i.e. using pre-GWT 1.6 with a project that has the WAR
     * Nature).
     */
    return StatusUtilities.OK_STATUS;
  }
  ...
  public void removeGWTSdkForFacet(IProject project) throws BackingStoreException, CoreException {
    ...
    ClasspathUtilities.replaceContainerWithClasspathEntries(JavaCore.create(project), GWTRuntimeContainer.CONTAINER_ID);
